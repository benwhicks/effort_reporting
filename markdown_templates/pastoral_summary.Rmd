...
student.info <- student.info %>% mutate(Student.name = trim_pref_name(paste(Firstname, Lastname)))
df.house.summary <- merge(df.teacher, student.info[,c("Student.code","Student.name","House","Cohort","Gender")])
df.house.summary <- df.house.summary[df.house.summary$House %in% houses,]

df.cohort.percentiles <- df.house.summary %>% group_by(Cohort, Date) %>% mutate(pr = 100*percent_rank(Effort))
