...
effort.path <- paste0(datdir, "2017 Term 4 Effort Data.csv")
df <- read.csv(effort.path)
df$Timestamp <- as.POSIXct(df$Timestamp)

# hack
#gdups <- function(x){
#  x[duplicated(x[,-which(names(x) %in% c("Timestamp", "Score"))]),]
#}
#gdups(df)
#df <- aggregate(Timestamp ~ ., data = df, max) # timestampe filter - shouldn't be needed but is
# end hack

student.info <- read.csv(paste0(datdir, "edumate_info_2017t4.csv"))
student.info <- subset(student.info, select = c("Student.code", "Student.name", "Gender", "Cohort"))
...
Effort analysis of 7 - 12 effort collected in Term 4 2017. Report generated `r Sys.Date()`
...
gg +facet_grid(. ~ Cohort) + ggtitle("Individual score camparison by Year level: Student - Teacher") + 
